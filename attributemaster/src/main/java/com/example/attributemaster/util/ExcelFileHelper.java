package com.example.attributemaster.util;
...
import com.example.attributemaster.exception.ExcelFileException;
...
public class ExcelFileHelper extends AbstractExporter {
	...
	public void exportData(HttpServletResponse response, String sheetName, Class<?> clazz, String contentType,
		...
		writeHeaderLine(sheetName, clazz);
		if (list.isEmpty()) {
			throw new ExcelFileException("No data available in the database for export.");
		}
		writeDataLines(list);
	...
	private void writeHeaderLine(String sheetName, Class<?> clazz) {
		...
		if (sheet == null) {
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				if ("dynamicFields".equals(field.getName())) {
					// Skip "dynamicFields" in header
					continue;
				}
	...
	private void writeDataLines(List<?> list) throws ExcelFileException {
		...
		for (Object obj : list) {
			...
			for (Field field : fields) {
				if ("dynamicFields".equals(field.getName())) {
					// Skip "dynamicFields" in data lines
					continue;
				}
				try {
	...
	public <T> List<T> readDataFromExcel(InputStream inputStream, Class<T> clazz)
		...
		validateExcelFile(sheet.getRow(0), clazz);
	...
	private void validateExcelFile(Row headerRow, Class<?> clazz) throws ExcelFileException {
		List<Field> fields = getCachedFields(clazz);
		...
		if (headerCellCount > fields.size() - dynamicFieldsCount) {
			throw new ExcelFileException("Excel file contains extra columns.");
		}
